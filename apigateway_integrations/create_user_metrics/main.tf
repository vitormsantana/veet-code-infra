...
resource "aws_api_gateway_resource" "create_user_metrics" {
  rest_api_id = var.rest_api_id
  parent_id   = var.parent_id
  path_part   = "create_user_metrics"
}

resource "aws_api_gateway_method" "post" {
  rest_api_id   = var.rest_api_id
  resource_id   = aws_api_gateway_resource.create_user_metrics.id
  http_method   = "POST"
...
resource "aws_lambda_permission" "api_gateway" {
  statement_id  = "AllowAPIGatewayInvokeCreateUserMetrics"
  action        = "lambda:InvokeFunction"
  function_name = var.lambda_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "arn:aws:execute-api:sa-east-1:149536475122:${var.rest_api_id}/*/*/create_user_metrics"
}

output "deployment_trigger" {
  value = sha1(jsonencode({
    resource     = aws_api_gateway_resource.create_user_metrics.id
