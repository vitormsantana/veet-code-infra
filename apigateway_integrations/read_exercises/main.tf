...
resource "aws_api_gateway_resource" "read_exercises" {
  rest_api_id = var.rest_api_id
  parent_id   = var.parent_id
  path_part   = "read_exercises"
}

resource "aws_api_gateway_method" "get" {
  rest_api_id   = var.rest_api_id
  resource_id   = aws_api_gateway_resource.read_exercises.id
  http_method   = "GET"
...
resource "aws_lambda_permission" "api_gateway" {
  statement_id  = "AllowAPIGatewayInvokeReadExercises"
  action        = "lambda:InvokeFunction"
  function_name = var.lambda_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "arn:aws:execute-api:sa-east-1:149536475122:${var.rest_api_id}/*/*/read_exercises"
}

output "deployment_trigger" {
  value = sha1(jsonencode({
    resource     = aws_api_gateway_resource.read_exercises.id
